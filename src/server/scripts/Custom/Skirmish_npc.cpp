...
public:
    ...
    bool OnGossipSelect(Player *pPlayer, Creature * /*pCreature*/, uint32 /*sender*/, uint32 action)
    {
        ...
        WorldPacket data; // send in queue OPCODE
...
public:
    ...
    static bool HandleSpectateFromCommand(ChatHandler* handler, const char *args)
    {
        ...
        {
            handler->PSendSysMessage("Can`t do that. Arena didn`t start.");
            handler->SetSentErrorMessage(true);
            return false;
        }
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Spectate specific player", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_PLAYER, "", 0, true);
